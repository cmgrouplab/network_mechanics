...
//all periodic neighbors are removed, this may lead to unconstrained node, but should be fine using relaxation method
...
double get_dist_finite(int index1, int index2)
{
      double dx = node_coords[index1][0] -  node_coords[index2][0];  
      double dy = node_coords[index1][1] -  node_coords[index2][1]; 
      double dz = node_coords[index1][2] -  node_coords[index2][2];
...
void read_config()
{
     ...
     for(int i=0; i<N_node; i++)
     {
         ...
         for(int j=0; j<N_node; j++)
         {
             ...
             //remove the periodic neighbors based on the separation distances...
             if(connect_map[i][j] == 1 && get_dist_finite(i, j)<0.5*L)
             {
         ...
         //&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
         //another check, to identify node with peridoci neighbors
         if(temp_neigh_ct != neigh_num[i])
         {
             //cout<<"neighbor number is wrong! node = "<<i<<endl;
             //exit(1);
             
             cout<<"node "<<i<<" had periodic neighors, neigh_num = "<<neigh_num[i]<<"; temp_neigh_ct = "<<temp_neigh_ct<<endl;
             neigh_num[i] = temp_neigh_ct;
...
int main()
{
  ...
  get_dist0(); //check the correctedness of the neighbor list
