...
//modified: 06/30/2017
//author: yang.jiao.2@asu.edu
//It turns out the periodic BC does not lead to nice results in the analysis
//In this version, still use a finite-sized RVE
//Under uniaxial loading, the two opposite boundary nodes along the loading direction is fixed
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
//make sure the fixed boundary function is consistent with the given BC
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
int N_loading = 1; //number for loading steps
double strain_xx = 0.1; // the normal strain
double strain_yy = 0.0;
double strain_zz = 0.0; 
//using different combination of the strain, to model different loading mode


int N_relax = 8000; //number of relaxation after each loading
...
double force_threshold = 0.1; //to check agains f-f_min/f_max-f_min, to visualize the force chains
...
//this is to fix boundaries along all three orthogonal directions
...
//remove periodic BC in this case
	  ...
	  for(int m=0; m<n_segment; m++)
	    {
	      ...
	      //double temp_force_ratio = fabs((force_mod[i][j])/(force_max));
	      //cout<<"force-raito = "<<temp_force_ratio<<endl;
...
int main()
{
   ...
   // get_cell(); //put in the cell
    ...
    //This has been to after get_force_PDF(), to get values for f_max and f_min
